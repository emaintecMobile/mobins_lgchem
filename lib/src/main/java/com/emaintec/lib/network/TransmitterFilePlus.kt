...
class TransmitterFilePlus(
	url: String,
	private val action: String? = null,
	private val RESULT_DATA: String? = null,
	...
	override fun process(httpURLConnection: HttpURLConnection): String? {
		...
		try {
			...
			writer.append("--$BOUNDARY").append(LINE_FEED)
			writer.append("Content-Disposition: form-data; name=\"RESULT_DATA\"").append(LINE_FEED)
			writer.append("Content-Type: text/plain; charset=$CHARSET").append(LINE_FEED)
			writer.append(LINE_FEED)
			writer.append(RESULT_DATA).append(LINE_FEED)
			writer.flush()
			/* 파일 데이터를 넣는 부분 */
			writer.append("--$BOUNDARY").append(LINE_FEED)
			...
			if (responseCode != HttpURLConnection.HTTP_OK && responseCode != HttpURLConnection.HTTP_CREATED) {
				...
				errMessage = "{\"resultSts\":\"F\", \"resultMsg\":\"$stringBuilder\"}"
			}
