...
class commonGridAdapter<T>() : RecyclerViewAdapter<T, commonGridAdapter.MyViewHolder>() {
    ...
    fun setOnCellClickListener(listener: commonGridAdapter.OnCellClickListener?) {
    ...
    fun swapItems(fromPosition: Int, toPosition: Int) {
        if (fromPosition < toPosition) {
            for (i in fromPosition..toPosition - 1) {
                _arrayList.set(i, _arrayList.set(i+1, _arrayList.get(i)));
            }
        } else {
            for (i in fromPosition..toPosition + 1) {
                _arrayList.set(i, _arrayList.set(i-1, _arrayList.get(i)));
